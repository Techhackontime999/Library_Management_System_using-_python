    ...
    def return_book(self):
        try:
            ...
            if book_details:
                ...
                # Reinsert into books table
                con.execute('''
                    INSERT INTO books (book_id, book_name, book_price, book_edition, book_category, publisher_name, publication_year)
                    SELECT book_id, book_name, book_price, book_edition, book_category, publisher_name, publication_year FROM returnbook WHERE book_id=?
                ''', (book_id,))
                con.execute('DELETE FROM returnbook WHERE book_id=?', (book_id,))
                con.commit()
            else:
        ...
        try:
            ...
            if mode == "return":
                ...
                if book_details:
                    ...
                    con.execute('DELETE FROM returnbook WHERE book_id=?', (book_id,))
                    con.commit()
                    print("Book returned and re-added to books table.")
                else:
                    print("Book not found in books.")
        except s.Error as e:
